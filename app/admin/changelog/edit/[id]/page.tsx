...
import { ChangeType, ChangelogEntry } from "@/app/news/changelog";
import { useRouter } from "next/navigation";
import { FaPlus, FaTrash, FaArrowLeft } from "react-icons/fa";
import { use } from "react";
import Link from "next/link";

// Funkcja do parsowania daty z formatu DD.MM.YYYY na obiekt Date
...
// Funkcja do konwersji daty z DD.MM.YYYY na YYYY-MM-DD (dla input type="date")
...
export default function EditChangelog({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchChangelog = async () => {
      try {
        ...
        const entry = data.changelog[id] as ChangelogEntry;

        if (entry) {
          setDate(convertDateForInput(entry.date));
          setVersion(entry.version || "");
          setMode(entry.mode || "Strona");
        ...
        alert('Wystąpił błąd podczas pobierania wpisu');
        router.push('/admin/changelog');
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(data.error || 'Wystąpił błąd podczas aktualizacji wpisu');
      }
    } catch (error) {
    ...
    return (
      <div className="p-6">
        <div className="flex justify-center items-center h-[200px]">
          <div className="text-gray-400">Ładowanie wpisu...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Edytuj wpis changelogu</h1>
        <Link
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="flex justify-end">
          ...
          >
            {submitting ? "Aktualizowanie..." : "Aktualizuj wpis"}
          </button>
        </div>
      </form>
    </div>
